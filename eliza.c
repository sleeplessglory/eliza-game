...
char *read1(int x) { //keywords
    char *r1 = data1[x];
    x += 1;
    return r1;
}

char *read2(int x) { //conjugations
    char *r2 = data2[x];
    x += 1;
    return r2;
}

char *read3(int x) { //replies
    char *r3 = data3[x];
    x += 1;
    return r3;
}
...
void inp(char *I, char *P, int L);

void goto170(char *I, char *P, int L) {
    inp(I, P, L);
    //    rep(I, P, L);
}

void goto390(int x, int n1, char *z) {
    for (x = 0; x < n1; x++) {
        z = read1(x);
    }
}
...
*/
void restore(int x, char *z, int n1, int n2, int n3) {
...
void inp(char *I, char *P, int L) {
    ...
    for (L = 0; L < strlen(I); L++) {
        if (I[L] == '\'') { //??? 230
            I = I.substr(0, L) + I.substr(L + 1, strlen(I) - L - 1); //substr(позиция, длина)
        }
        if (L + 4 <= strlen(I)) { //L+5?
...
int main() {
    ...
    tab(18);
    PRINT("MORRISONTOWN, NEW JERSEY \n \n \n \n");
    
    int s[36], r[36], n[36], n1 = 36, n2 = 14, n3 = 112, L, S, T, k, x;
    char *z, I, P, ks, F, C, Ss, R;
    bool G390;
    ...
    while (I != "SHUT") {
        ...
        if (S > 0) {
            ...
            C = " " + I.substr(strlen(F) + L, strlen(I) - strlen(F) - L + 1) + " ";//OOR
            //C = " " + I.substr(I.length() - (I.length() - F.length() - L), I.length() - F.length() - L + 1) + " ";//OOR
            for (x = 0; x < n2 / 2; x++) { //data123?
                Ss = read1(x);
                R = read1(x);
                for (L = 0; L < strlen(C); L++) {
                    if (L + strlen(Ss) > strlen(C)) {
                        if (L + strlen(R) > strlen(C)) {
                            continue;
                        }
                    }
                    if (C.substr(L, strlen(Ss)) != Ss) {
                        if (L + strlen(R) > strlen(C)) {
                            continue;
                        }
                    }
                    C = C.substr(0, L) + R + C.substr(strlen(C) - (strlen(C) - strlen(Ss) - L), strlen(C) - strlen(Ss) - L + 1); //L-1 длина?
                    L += strlen(R);
                    //530???????????????????????
                }
            }
            if (C.substr(1, 1) == " ") {
                C = C.substr(strlen(C) - 2, 2); //1 space?
            }
            for (L = 0; L < C.length(); L++) {
                if (C.substr(L, 1) == "!") {
                    C = C.substr(0, L - 1) + C.substr(strlen(C) - (strlen(C) - L));
                }
            }
        }
        if (G390 == false) {
        ...
        if (F.substr(strlen(F) - 1, 1) != "*") { //OOR
            PRINT(F, "\n");
            P = I;
            continue;
        }
        //needed?
        PRINT(F.substr(strlen(F) - 2), C, "\n");
        P = I;   
    }
}
